...
export const testimonials = [
  {
    quote:
      "Working with Shipra Designs was an absolute pleasure. They understood our vision perfectly and transformed our house into a home that reflects our personality and lifestyle. Every detail was thoughtfully planned and executed to perfection.",
  ...
  {
    quote:
      "The dedication to timeline, immaculate material finishes, and flawless Vastu planning exceeded all our expectations. Our living spaces feel radiant, spacious, and truly elevated.",
  ...
  {
    quote:
      "From turnkey execution to custom furniture selections, Shipra's team handled everything effortlessly. It was hands down the smoothest interior design journey we've ever had.",
...
export function TestimonialsSection() {
  ...
  return (
    <section className="relative overflow-hidden bg-[#FBF9F5] py-14 md:py-20 font-sans selection:bg-[#c59a58]/20">
      {/* Ambient Botanical Leaves (Bottom Left & Right) */}
      ...
      <div className="mx-auto max-w-[1420px] px-5 sm:px-8 lg:px-12 relative">
        ...
        <div className="hidden xl:flex absolute left-4 top-24 -rotate-90 origin-top-left items-center gap-3 select-none">
          ...
          <span className="text-[10px] font-semibold tracking-[0.28em] uppercase text-[#B58544]">
            REAL SPACES, REAL STORIES
          </span>
          <span className="size-1 rounded-full bg-[#B58544]" />
        </div>

        {/* ================= TOP SECTION WITH HEADER & ARC HERO ================= */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative">
            ...
            <div className="mt-10 hidden lg:grid grid-cols-4 divide-x divide-[#E8DFC8]/80 max-w-lg">
              {stats.map((item) => (
                <div key={item.label} className="flex flex-col items-center px-3 text-center">
                  ...
                  <span className="font-serif text-[22px] font-normal leading-none text-[#1A1816]">
                    {item.value}
                  </span>
                  <span className="mt-1 text-[8.5px] font-medium tracking-[0.06em] text-[#7A7268] whitespace-nowrap">
                    {item.label}
                  </span>
                </div>
              ))}
          ...
          {/* Right Column: Signature Arched Swoop Hero (Desktop) */}
          <div className="hidden lg:block lg:col-span-6 relative h-[380px] w-full">
            {/* Double Arc Contour Lines */}
            ...
            {/* Masked Luxury Room Preview Image */}
            <div className="absolute top-4 right-0 h-[calc(100%-32px)] w-[515px] overflow-hidden rounded-tl-[220px] rounded-bl-[220px] shadow-lg">
              <Image
                ...
                alt="Luxury living room testimonial project"
                fill
                priority
        ...
        <div className="lg:hidden mt-8 grid grid-cols-2 gap-3 p-4 rounded-[20px] border border-[#E8DFC8] bg-white/70">
          {stats.map((item) => (
            <div key={item.label} className="flex items-center gap-3 p-2">
              ...
              <div>
                <span className="font-serif text-[18px] font-normal leading-none text-[#1A1816]">
                  {item.value}
                </span>
                <p className="text-[8px] font-medium tracking-tight text-[#7A7268] uppercase">
                  {item.label}
                </p>
              </div>
            </div>
          ...
          <div className="relative overflow-hidden rounded-[26px] border border-[#E8DFC8] bg-[#FAF8F5]/90 backdrop-blur-md p-8 md:p-12 shadow-[0_20px_50px_rgba(40,25,10,0.04)]">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              ...
              <div className="lg:col-span-7 flex flex-col justify-between">
                {/* Large Gold Quote Glyph */}
                <span className="font-serif text-[56px] text-[#B58544] leading-none select-none">
                  “
                </span>
                ...
                <div className="mt-8 flex flex-col">
                  <span className="font-sans text-[15px] font-bold text-[#1A1816]">
                    {current.name}
                  </span>
                  <span className="mt-1 text-[9px] font-bold tracking-[0.24em] uppercase text-[#B58544]">
                    {current.role}
                  </span>
                </div>

                {/* Desktop Dots & Nav Buttons */}
                <div className="mt-10 flex items-center gap-4">
                  <button
                    type="button"
                    onClick={goPrev}
                    className="flex size-10 items-center justify-center rounded-full bg-[#322F2C] text-white shadow hover:bg-black transition-colors"
                    aria-label="Previous"
                  >
                    <ArrowLeft size={15} />
                  </button>
                    ...
                    {testimonials.map((_, i) => (
                      <span
                        ...
                        className={`size-2 rounded-full transition-all duration-300 ${
                          i === index ? "bg-[#B58544] scale-125" : "bg-[#D5C8B4]"
                        }`}
                      />
                    ))}
                  </div>
              ...
              {/* Right Column: Layered Architectural Project Photo (Desktop) */}
              <div className="hidden lg:flex lg:col-span-5 justify-end relative">
                {/* Backdrop Textured Plinth Shape */}
                ...
                {/* Foreground Dining/Interior Photo */}
        ...
        {/* Mobile View: Clean Bottom Author Row with Arrow Toggles */}
        <div className="lg:hidden mt-4 flex items-center justify-between px-2">
          <button
            type="button"
            onClick={goPrev}
            className="flex size-9 items-center justify-center rounded-full border border-[#D5C8B4] bg-white text-[#7A7268] shadow-sm active:scale-95"
            aria-label="Previous"
          >
            <ArrowLeft size={14} />
          </button>
